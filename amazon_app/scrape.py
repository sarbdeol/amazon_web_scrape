...
output_file_path = os.path.join('/root/amazon_web_scrape/media/download/', filename)
...
token = 'b8f5d522a81d4ef88687fa1e9fb1d35470e5f95b45b'#"2594e5d442b4457aa0e37b651200277726751d1e518"
...
def scrape_amazon(asin_value):
    ...
    if response.status_code == 200:
        ...
        availablity=soup.find('div',id="availability").get_text(strip=True)
        if 'unavailable' in availablity:
            print(f'{asin_value} unavailable')
...
def scrape_asin_batch(asin_values):
    for asin_value in asin_values:
        if asin_value:
            main_data=scrape_amazon(asin_value)
            try:
                add_data_to_csv(main_data)
...
def main(file_path, batch_size):
    ...
    data.save()
    shutil.move(file_path, '/root/amazon_web_scrape/media/done_files')
    print(f"File moved successfully to /root/amazon_web_scrape/media/done_files")
